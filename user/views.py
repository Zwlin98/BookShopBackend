...
from user.serializer import SmsCodeSerializer, LoginWithSmsCodeSerializer, UserRegisterSerializer, \
...
class SmsCodeView(CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            sms = SMS()
            sms.send_sms(phone_number, code)
        ...
        #         return Response({"status": "发送失败"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({"status": "发送异常"}, status=status.HTTP_400_BAD_REQUEST)
